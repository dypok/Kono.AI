...
/// Polling interval between inbound directory scans.
/// The event-driven watcher is owned by US-RUST-001; this poll keeps the
/// triage pipeline functional independently of that story.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let watch_dir = std::env::var("WATCH_DIR")
        .unwrap_or_else(|_| "/data/storage/inbound".into());
    let failed_dir = PathBuf::from(
    ...
    info!("🦀 [Kono Rust Core] Triage engine started, watching {}", watch_dir);
