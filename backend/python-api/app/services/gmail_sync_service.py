                            ...
                            if ext in SUPPORTED_EXTENSIONS or self._is_invoice_subject(subject):
                                ...
                                if payload:
                                    ...
                                    try:
                                        ...
                                        # 🛑 FILTRO ESTRICTO: Solo procesar e ingresar documentos que sean facturas / recibos confirmados
                                        if is_other:
                                            logger.info("Discarding non-invoice attachment from IMAP: %s (Type: OTHER, Score: %s)", decoded_filename, extracted.get("classifier_score"))
                                        ...
                                        if AsyncSessionLocal is not None:
                                            async with AsyncSessionLocal() as db:
                                                ...
                                                if not existing:
                                                    ...
                                                    await db.commit()
                                                    logger.info("Successfully ingested genuine IMAP invoice into PostgreSQL: %s (Doc ID: %s) label=%s", decoded_filename, doc_id, label)
