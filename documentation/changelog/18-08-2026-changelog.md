...
- [Tuesday]-[18/08/2026]-[20:22] : Fixed two PY-001 logic bugs found during integration validation (tests were failing in a clean env): (1) DeterministicTableParser.find_totals_section_top now takes header_bottom and ignores header-column words so a "Total" column header is not mistaken for the footer totals section; (2) INVOICE_FOLIO_PATTERN now captures multi-segment folios like INV-2026-001 instead of truncating to INV-2026. All 5 PY-001 unit tests now pass.
